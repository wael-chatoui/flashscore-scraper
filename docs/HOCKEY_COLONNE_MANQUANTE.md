...
La colonne **<=4 buts** (equivalent de la colonne O pour Team A) **n'existe pas** cote Team B.

- Team A a 4 colonnes de stats : O (<=4), P (<=5), Q (=6), R (>=7)
- Team B a seulement 3 colonnes de stats : W (<=5), X (=6), Y (>=7)

Il n'y a pas de colonne V pour "<=4 buts Team B" dans la structure actuelle de la feuille.
...
Pour ajouter cette donnee, il faudrait :
